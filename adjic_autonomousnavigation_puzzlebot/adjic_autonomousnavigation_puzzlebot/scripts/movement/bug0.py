...
class NavigateToGoal():
    def __init__(self):
        ...
        pub_closest = rospy.Publisher("closest_object", Marker, queue_size=1)
        ...
        while not rospy.is_shutdown():
            ...
            pub_closest.publish(marker_closest)
    ...
    def compute_gtg_control(self, x_target, y_target, x_robot, y_robot, theta_robot):
        ...
        kvmax = 1.0  # max linear speed gain
    ...
    def leave_fw(self, theta_gtg, theta_ao, d_t, d_t1):
        ...
        print()
        if (d_t < (abs(d_t1) - self.eps) and (np.abs(theta_ao - theta_gtg) < np.pi / 2)):
            return True
        else:
            return False
